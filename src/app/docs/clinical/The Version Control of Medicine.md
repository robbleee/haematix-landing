...
* **Output:** **Acute Myeloid Leukaemia (AML) with NPM1 mutation**.1
...
This discrepancy creates a massive data integrity problem. A patient diagnosed in a hospital using WHO 2022 guidelines is coded as a "Leukaemia" patient. The same patient in a hospital using ICC guidelines might be coded as "MDS." This affects cancer registry data, clinical trial eligibility, and insurance reimbursement codes.16 It fundamentally breaks the "version control" of medical data, making it impossible to compare cohorts across institutions without complex data normalization. This is where a Logic Engine becomes indispensable: it can act as a "translator," mapping patients to the correct diagnostic bucket based on the specific ruleset required for a given purpose.
