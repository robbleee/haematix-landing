...
const FlowchartDiagram = () => {
  ...
  const nodes = [
      ...
      { id: 'reports', x: 200, y: 280, width: 130, height: 50, type: 'rect', label: 'All Relevant Reports', explanation: 'Input data source: gathers the reports or structured findings selected for review.', dataExample: 'Case: local-001\nReports: Path_Report_12A.pdf, Mol_Report_34B.txt, Cyto_Report_56C.hl7...' },
      { id: 'nlp', x: 400, y: 280, width: 160, height: 50, type: 'rect', label: 'Data Structuring', explanation: 'Prepares report findings or manually entered values for clinician review.', dataExample: 'Reviewing text for keywords and values...' },
              { id: 'predefined', x: 400, y: 370, width: 160, height: 60, type: 'rect', label: ["Predefined Required", "Data Fields"], explanation: 'Specifies the key data points the workflow should make visible for review.', dataExample: "['Gene Mutation', 'VAF', 'Blast %', 'Cytogenetics', 'Prior Diagnosis Term']" },
      { id: 'extracted', x: 610, y: 280, width: 130, height: 50, type: 'rect', label: 'Reviewed Data Fields', explanation: 'Structured data fields available for clinician confirmation before criteria are applied.', dataExample: JSON.stringify({ gene: 'FLT3', mutation: 'ITD', vaf: '45%', blasts: '60%', cyto: '46,XX', context: 'AML review' }, null, 2) },

      // Treatment and Clinical Trial Path
      { id: 'treatmentEngine', x: 450, y: 10, width: 200, height: 50, type: 'rect', label: 'Consensus Pathway Review', explanation: 'Displays relevant consensus pathway context for clinician review where configured. It does not prescribe or replace local protocols.', dataExample: 'Review context:\n1. FLT3 marker present\n2. Local protocol review required\n3. MDT judgement remains final', style: { fill: '#E3F2FD', stroke: '#1976D2', strokeWidth: 2 } },
      { id: 'clinicalTrialEngine', x: 700, y: 10, width: 200, height: 50, type: 'rect', label: 'Clinical Trial Matching Support', explanation: 'Surfaces potentially relevant clinical trials for review based on molecular profile, disease characteristics, and eligibility criteria.', dataExample: 'Potential matches:\n1. NCT04293562: FLT3-ITD AML combination therapy\n2. NCT04462627: Novel FLT3 inhibitor Phase II\n3. Further eligibility confirmation required', style: { fill: '#E3F2FD', stroke: '#1976D2', strokeWidth: 2 } },

      // Reasoning Path
      { id: 'allData', x: 625, y: 90, width: 100, height: 100, type: 'circle', label: 'Reviewed Data', explanation: 'A central view of clinician-confirmed structured data used by the criteria workflow.', dataExample: JSON.stringify({ case_id: 'local-001', report_ids: ['12A', '34B', '56C'], reviewed_fields: { gene: 'FLT3', mutation: 'ITD', vaf: '45%' }, demographic: { age: 55, sex: 'F'}, lab_values: { wbc: '15.3', hgb: '9.8' } }, null, 2)},
      { id: 'reasoning', x: 860, y: 115, width: 140, height: 50, type: 'rect', label: 'Rule Review', explanation: 'Applies deterministic rules and makes the intermediate reasoning visible for review.', dataExample: 'Applying visible WHO/ICC/ELN criteria...' },
      ...
      { id: 'mrdReview', x: 1200, y: 80, width: 150, height: 40, type: 'roundedRect', label: 'MRD Context Review', explanation: 'Output review related to Minimal Residual Disease context.', dataExample: 'Context: MRD status may affect local follow-up pathway.', style: { fill: '#E3F2FD', stroke: '#1976D2', strokeWidth: 2 } },
      { id: 'geneReview', x: 1200, y: 130, width: 150, height: 40, type: 'roundedRect', label: 'Gene Review', explanation: 'Output review focusing on genetic markers or mutations.', dataExample: 'Finding: FLT3-ITD mutation identified for clinician review.', style: { fill: '#E3F2FD', stroke: '#1976D2', strokeWidth: 2 } },
      ...
      { id: 'classificationModel', x: 810, y: 280, width: 140, height: 50, type: 'rect', label: 'Criteria Engine', explanation: 'Applies deterministic classification criteria to clinician-reviewed data.', dataExample: 'Input: { blasts: 60, cyto: \'46,XX\', gene: \'FLT3\', ...}\nApplying WHO 2022 / ICC 2022 logic...' },
      ...
      { id: 'decision', x: 830, y: 410, width: 100, height: 80, type: 'diamond', label: 'AML or MDS', explanation: 'Criteria branch: evaluates whether AML or MDS pathway logic applies for review.', dataExample: 'Condition: blasts (60%) >= 20%\nCriteria branch: AML' },
      { id: 'ipcc', x: 1020, y: 385, width: 150, height: 60, type: 'rect', label: ["IPCC Risk", "Stratification Model"], explanation: 'Risk model applied if the MDS pathway applies (uses International Prognostic Scoring System criteria).', dataExample: 'N/A for this AML case.' },
